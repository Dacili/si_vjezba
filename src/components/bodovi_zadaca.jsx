...
class BodoviZadaca extends Component {
    ...
    */
    ...
    unioBodove = () => {
        ...
        var noviNiz = [];
        for (let i = 1; i <= this.props.broj_zad; i++) {
            if (document.getElementById(i).value !== "") {//inace bude NaN
    ...
    render() {
        ...
        return (
            <div>
                <div className="card-header bg-primary text-light"><h4><b>Bodovi zadaće</b></h4></div>
                <div className="card-body">
                    <h4 className="card-title">Želim da svi zadaci imaju jednak maksimalan broj bodova.</h4>
                    <div className="form-group">
                        <div className="custom-control-static custom-switch">
                            ...
                            <label className="custom-control-label" htmlFor="customSwitch1">DA</label>
                            <br></br>
                            <br></br>
                        </div>
                        <h5 className="card-title">Broj bodova:</h5>
                        <input id="brbod" type="text" className="form-control-static" onChange={this.toggle_klik}></input>
                        <button type="button" className="btn btn-secondary ml-3 " onClick={this.klik_isti_br_bod}>OK</button>
                        <hr></hr>
                    </div>
                </div>
                <div id="tabela" >
                    <table className="table table-bordered text-center bg-active border-solid">
                        <thead>
                            <tr className="bg-primary text-light">
                                {kolone.map(
                                    jedno => <th scope="col" key={jedno + 200}>{jedno}</th>)}
                            </tr>
                        </thead>
                        <tbody>
                            <tr className="bg-secondary text-light">
                                {kolone.map(
                                    jedno => <th scope="col" key={jedno}><input type="text" className="form-control bg-primary text-light text-bold" id={jedno} onChange={this.unioBodove}></input></th>
                                )}
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <span className="badge badge-primary ml-3">
                        <h5 id="ukupnoStanje">{this.state.ukupnoBodovaZadaca}</h5></span>
                    <hr></hr>
                </div>
            </div>
        );
